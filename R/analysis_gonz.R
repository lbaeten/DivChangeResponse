...
gonz_fit <- lmer(log_SR_ratio ~ Duration + (1+Duration|Study), data=dta_SR, REML=T)
summary(gonz_fit)

# Predictions
pred <- expand.grid(Duration=c(seq(from=min(dta_SR$Duration),to=max(dta_SR$Duration),by=1)),log_SR_ratio=0)
...
# Plot the results
gonzPlot <- qplot(Duration, log_SR_ratio, data=dta_SR, size=I(2)) +
